...
int MiddleElement(PNODE First)
{
    ...
    while((Teacher != NULL) && (Teacher->next != NULL))  // For Even and Odd number of nodes
    {
        Teacher = Teacher->next->next; // 1 // This order should not be changed
        Student = Student->next;       // 2 // This order should not be changed
    }
